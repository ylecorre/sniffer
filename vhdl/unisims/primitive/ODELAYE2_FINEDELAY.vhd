...
--  /   /                  Output Fixed or Variable Delay Element with Fine Adjustment
-- /___/   /\     Filename : ODELAYE2_FINEDELAY.vhd
...
architecture ODELAYE2_FINEDELAY_V OF ODELAYE2_FINEDELAY is
...
--signal	GSR_ipd		: std_ulogic := 'X';
  ...
  signal        OFDLY_ipd       : std_logic_vector(2 downto 0) := (others => '0');
  ...
  signal        OFDLY_dly       : std_logic_vector(2 downto 0) := (others => '0');
...
begin
  ...
  C_dly          	 <= C xor IS_C_INVERTED              	after 0 ps;
  ...
  ODATAIN_dly    	 <= ODATAIN  xor IS_ODATAIN_INVERTED      	after 0 ps;
  OFDLY_dly      	 <= OFDLY          	after 0 ps;
...
--####################################################################
  ...
  begin
     ...
     if((CINVCTRL_SEL /= "TRUE") and (CINVCTRL_SEL /= "FALSE"))then
          assert false
          report "Attribute Syntax Error: The attribute CINVCTRL_SEL on ODELAYE2_FINEDELAY must be set to either TRUE or FALSE."
          severity Failure;
     end if;
    ...
    --------DELAY_SRC check
     if((DELAY_SRC /= "ODATAIN") and (DELAY_SRC /= "CLKIN"))then
          assert false
          report "Attribute Syntax Error: The attribute DELAY_SRC on ODELAYE2_FINEDELAY must be set to either ODATAIN or CLKIN."
          severity Failure;
     end if;
     ...
     if((HIGH_PERFORMANCE_MODE /= "TRUE") and (HIGH_PERFORMANCE_MODE /= "FALSE"))then
          assert false
          report "Attribute Syntax Error: The attribute HIGH_PERFORMANCE_MODE on ODELAYE2_FINEDELAY must be set to either TRUE or FALSE."
          severity Failure;
     end if;
     ...
     if((PIPE_SEL /= "TRUE") and (PIPE_SEL /= "FALSE"))then
          assert false
          report "Attribute Syntax Error: The attribute PIPE_SEL on ODELAYE2_FINEDELAY must be set to either TRUE or FALSE."
          severity Failure;
     end if;
...
--#####                Assign Fine Delays                        #####
--####################################################################
  prcs_AssignFineDelays:process(OFDLY_dly, fine_delay_0, fine_delay_1,
