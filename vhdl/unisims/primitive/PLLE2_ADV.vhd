...
-- Input/Output Pin signals
        ...
        signal IS_CLKINSEL_INVERTED_BIN : std_ulogic := IS_CLKINSEL_INVERTED;
  ...
  if (rising_edge(pll_locked_tmp1)) then
    if (clkinsel_in = '0') then
       ...
       if (period_avg > pchk_tmp1 or  period_avg < pchk_tmp2) then
         assert false report "Warning : input CLKIN2 period and attribute CLKIN2_PERIOD are not same." severity error ;
       end if;
    else
       pchk_tmp1 := CLKIN1_PERIOD * 1100 ps;
       pchk_tmp2 := CLKIN1_PERIOD * 900 ps;
       if (period_avg > pchk_tmp1 or  period_avg < pchk_tmp2) then
         assert false report "Warning : input CLKIN1 period and attribute CLKIN1_PERIOD are not same." severity error ;
       end if;
    end if;
  end if;
  end process;
  ...
  begin
    if (rst_in = '1') then
       ...
       if (clkin_hold_f = 1) then
         ...
         assert FALSE  report " Warning : input CLKIN1 of PLLE2_ADV is stopped. Reset is required for PLLE2_ADV when input clock returns." severity error;
         else
         assert FALSE report " Warning : input CLKIN2 of PLLE2_ADV  is stopped. Reset is required for PLLE2_ADV when input clock returns." severity error;
         end if;
       end if;
    end if;
